...
export const NOT_AN_OPENING = new Set([
  "results_notice",
  "cancellation",
  "corrigendum",
  "exam_material",
...
/* Drop by NAME, never by "keep only advertisement". `document_class` arrived
 * in commoner-probe 0.19.0, so every record written before it is null.
 * Keeping only `advertisement` would erase the whole history. */
export function isAnOpening(ad = {}) {
  return !NOT_AN_OPENING.has(ad.document_class);
}
