...
// A null closing_date means two different things and the site used to show
// only one of them. `rolling` is the document SAYING there is no deadline.
// `not_found` and `not_examined` mean nobody read one. Showing an unread
// deadline as an affirmative rolling call tells a candidate they can apply at
// any time, which may be false and may cost them the post.
//
// A null status predates commoner-probe 0.18.0, so it cannot be resolved
// either way. It reads as unknown, not as rolling.
export function statesNoDeadline(ad) {
  return ad.closing_date_status === "rolling";
}

export function noDeadlineLabel(ad) {
  return statesNoDeadline(ad) ? "rolling, no deadline" : "deadline not stated";
}
...
export function renderAd(ad) {
  ...
  const deadlinePill = ad.closing_date
    ...
    : `<span class="deadline-pill no-dl">${escapeHTML(noDeadlineLabel(ad))}</span>`;
  ...
  if (ad.closing_date) {
    ...
    deadlineHTML = statesNoDeadline(ad)
      ? `<div class="deadline-rolling">⟳ Rolling</div>`
      : `<div class="deadline-rolling deadline-unknown">Deadline not stated</div>`;
  }
